...
$username = DB::fetch_all("SHOW COLUMNS FROM %t", array('appbyme_special_users'));
$usernamearray = mysqltoarray($username);
if (!in_array('type', $usernamearray)) {
    $sql1 = <<<EOF
ALTER TABLE `cdb_appbyme_special_users` ADD `type` TINYINT NOT NULL DEFAULT '0' AFTER `time`;
EOF;
    runquery($sql1);
}
